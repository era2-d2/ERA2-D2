...
void PIDEncoder() {
  ...
  if (PWME1 < 150) PWME1 = 150;

  if (PWMD1 > 255) PWMD1 = 255;
  if (PWMD1 < 150) PWMD1 = 150;
  ...
  if (PWME < 150) PWME = 150;

  if (PWMD > 255) PWMD = 255;
  if (PWMD < 150) PWMD = 150;
...
int ultimoEstadoX = 0, ultimoEstadoY = 0, ultimoEstadoGarra;
...
void Garra(int x, int y, bool fechada = false) {
  ...
  if (fechada == true) {
    ...
    for (int i = ultimoEstadoGarra; i >= 90; i--) {
      servo3.write(i);
      delay(10);
    }
  }
